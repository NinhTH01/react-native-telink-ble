...
import com.react.telink.ble.app.TelinkBleActivity
import com.react.telink.ble.app.TelinkBleApplication
import com.react.telink.ble.dto.*
import com.react.telink.ble.models.*
import com.react.telink.ble.models.json.MeshStorageService
import com.react.telink.ble.types.TelinkBleEvent
...
open class TelinkBleModule(reactContext: ReactApplicationContext) :
  ReactContextBaseJavaModule(reactContext),
  TelinkBleEventEmitter,
  TelinkBleEventHandler {
  companion object {
    ...
    if (nodes != null) {
      for (node in nodes) {
        if (node.onOffDesc != "OFFLINE") {
          ...
          nodeInfo.meshAddress = node.meshAddress
          nodeInfo.uuid = node.deviceUUID.hexString
          result.add(nodeInfo)
        }
      }
  ...
  override fun onKeyBindSuccess(event: BindingEvent) {
    ...
    if (currentDevice!!.state == NetworkingState.BINDING) {
      ...
      val bleDevice = BleDeviceDto()
      val manufacturerData = advertisingDevice.scanRecord.hexString

      bleDevice.manufacturerData = manufacturerData
      bleDevice.deviceType = manufacturerData.substring(147, 147 + 8)
      bleDevice.version = manufacturerData.substring(165, 165 + 5)
      bleDevice.uuid = pvDevice.nodeInfo.deviceUUID.hexString
      ...
      if (ActivityCompat.checkSelfPermission(
        ...
        val activity: TelinkBleActivity = currentActivity as TelinkBleActivity
        activity.requestPermissions(activity.permissions, TelinkBleActivity.REQUEST_PERMISSION_CODE)
        return
      }
      bleDevice.name = advertisingDevice.device.name
      bleDevice.macAddress = advertisingDevice.device.address
      bleDevice.meshAddress = pvDevice.nodeInfo.meshAddress
      bleDevice.rssi = advertisingDevice.rssi
  ...
  open fun turnOnBluetooth() {
    ...
    if (bluetoothAdapter != null) {
      if (!bluetoothAdapter.isEnabled) {
        ...
        val activity: TelinkBleActivity = currentActivity as TelinkBleActivity
        reactApplicationContext.startActivityForResult(enableBtIntent, REQUEST_ENABLE_BT, Bundle())
      }
    }
  }
  ...
  fun getStatus(meshAddress: Int, promise: Promise) {
    ...
    val rspMax: Int = TelinkBleApplication.getInstance().getMeshInfo()!!.onlineCountInAll
    val appKeyIndex: Int = meshInfo.defaultAppKeyIndex
  ...
  private fun setNextModel(promise: Promise) {
    ...
    val failDevice = FailedDeviceDto();
    failDevice.uuid = deviceInfo?.deviceUUID?.hexString
    failDevice.macAddress = deviceInfo?.macAddress
    failDevice.meshAddress = deviceAddress
    failDevice.groupAddress = groupAddress
    ...
    if (!MeshService.getInstance().sendMeshMessage(groupingMessage)) {
      delayHandler.removeCallbacksAndMessages(null)
      promise.reject("SetGroupFail", "SetGroupFail $deviceAddress", failDevice)
    } else {
      if (opType == 0) {
  ...
  fun getMeshNetwork(promise: Promise) {
    ...
    val meshNetworkInfo = MeshNetworkDto()

    meshNetworkInfo.appKey = Arrays.bytesToHexString(meshNetwork!!.appKeyList[0].key, "")
    meshNetworkInfo.netKey = Arrays.bytesToHexString(meshNetwork.meshNetKeyList[0].key, "")
  ...
  fun exportMeshNetwork(database: String, promise: Promise) {
    ...
    try {
      ...
      val fileCreated = FileCreatedDto()

      fileCreated.filePath = file.absolutePath
      fileCreated.fileName = fileName
  ...
  private fun onOtaDeviceSuccess(event: GattOtaEvent) {
    ...
    val otaContent = OtaProgressDto();
    otaContent.description = desc
    otaContent.progress = progress
  ...
  override fun onReceiveNotificationMessageUnknown(notificationEvent: NotificationMessage) {
    val notification = UnknownNotificationDto()

    notification.src = notificationEvent.src
    notification.dst = notificationEvent.dst
    notification.opcode = notificationEvent.opcode
    notification.params = notificationEvent.params.hexString
